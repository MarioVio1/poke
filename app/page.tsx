    ...
    setTimeout(() => {
      ...
      setGs(prev => ({
        ...
        party: prev.party.map(p => p === bestia ? {
          ...
          name: evolved.name,
          types: evolved.types,
          maxHp: Math.floor(evolved.baseStats.hp * 1.5),
          hp: Math.min(Math.floor(evolved.baseStats.hp * 1.5), p.hp),
          atk: Math.floor(evolved.baseStats.attack * 1.5),
          def: Math.floor(evolved.baseStats.defense * 1.5),
          spd: Math.floor(evolved.baseStats.speed * 1.5),
          moves: evolved.moves,
        } : p),
        inv: prev.inv.map(i => i.item.name === item.name ? { ...i, qty: i.qty - 1 } : i).filter(i => i.qty > 0)
      }))
